...
Base unit: 4px. Use these steps to keep rhythm consistent.
...
- Eyebrow label: uppercase, tracking-wide, mutedForeground.
- Title: Outfit 700 or 800, max 2 lines.
- Supporting text: Plus Jakarta Sans 400 or 500.

### Primary CTA row

- Primary button on the left, secondary button on the right.
- Optional microcopy below in mutedForeground.

### Feature grid

- 3-up card grid on desktop, stacked on mobile.
- Alternate card header colors: accent, secondary, tertiary.
- Decorative dashed connector line behind cards.

### Stats strip

- 3 to 4 stats, each in a sticker card.
- Big numeric value in accent or secondary with label below.

### Form layout

- Labels above inputs, helper text below.
- Group related fields inside a sticker card.
- Primary action anchored to the bottom right on desktop, full width on mobile.

### Empty state

- Use a large circle or blob with a simple icon inside.
- Add a short title, one-line explanation, and a single primary CTA.

### Status banner

- Left icon in a colored circle, text on the right.
- Use secondary for info, tertiary for warning, quaternary for success.
```
