...
export function useGestures(send: (event: any) => void) {
	const { sensitivity, scrollSpeed, leftHandMode } = useSettingsStore();
	...
	const masterPan = Gesture.Pan()
		...
		.onUpdate((e) => {
			...
			switch (mode.current) {
				case "move":
					moveChannel.push(dx * sensitivity, dy * sensitivity);
					break;
				...
				case "scroll": {
					...
					const SCROLL_SCALE = 0.05 * scrollSpeed;
				...
				case "swipe4": {
					...
					if (Math.abs(tx) > threshold || Math.abs(ty) > threshold) {
						...
						if (Math.abs(tx) > Math.abs(ty)) {
							send(
								Events.gesture((prefix + (tx > 0 ? "right" : "left")) as any),
							);
						} else {
							send(Events.gesture((prefix + (ty > 0 ? "down" : "up")) as any));
						}
					}
					break;
				}
